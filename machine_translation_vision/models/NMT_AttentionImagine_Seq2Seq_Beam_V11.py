#Optimized V9 with access to the images during Validation and Testing.
import torch
...
class NMT_AttentionImagine_Seq2Seq_Beam_V11(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 im_feats_size, \
        ...
        super(NMT_AttentionImagine_Seq2Seq_Beam_V11,self).__init__()
    ...
    def forward(self,src_var,src_lengths,tgt_var,im_var,teacher_force_ratio=1.0,max_length=80, criterion_mt=None, criterion_vse=None):
        ...
        decoder_hidden = F.tanh(self.decoderini(self.init_split*encoder_concat+(1-self.init_split)*encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))).unsqueeze(0)
    ...
    def beamsearch_decode(self,src_var,src_lengths,im_var,beam_size=1,max_length=80,tgt_var=None):#Now we have access to Image
        ...
        #Compute the concate encoder hidden states
        _,encoder_concat = self.vse_imagine(im_var,encoder_outputs,context_mask=context_mask)
