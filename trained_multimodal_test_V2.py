...
model_folder = "/home/zmykevin/machine_translation_vision/code/mtv_trained_model/WMT17/nmt_imagine_FR_16_4"
model_path = os.path.join(model_folder,"nmt_trained_imagine_model_best_BLEU.pt")
loss_model_path = os.path.join(model_folder,"nmt_trained_imagine_model_best_loss.pt")
...
output_path = "/home/zmykevin/machine_translation_vision/code/mtv_trained_model/Multi30K_Long_Subset/nmt_imagine_FR_16_4"
#output_path = "/home/zmykevin/machine_translation_vision/code/mtv_trained_model/MSCOCO17/nmt_imagine_FR_16"
...
test_prediction_path = os.path.join(output_path,'test_best_BLEU.'+target_language)
...
print("Full evaluation results with best Model:")
...
########################################Evaluate with best trained loss model######################################
#Load the model
...
#Evaluate the Image Retrieval Results
test_sample_size = len(test_data_index)
lim,ltxt = torch.FloatTensor(test_sample_size,best_loss_model.shared_embedding_size),torch.FloatTensor(test_sample_size,best_loss_model.shared_embedding_size)
if use_cuda:
    lim = lim.cuda()
    ltxt = ltxt.cuda()
...
#Save the translation prediction to the trained_model_path
test_prediction_path = os.path.join(output_path,'test_best_LOSS.'+target_language)
...
########################################Evaluate with best trained METEOR model######################################
#Load the model
best_meteor_model = torch.load(meteor_model_path)
...
test_r1,test_r5,test_r10,test_medr = im_retrieval_eval.t2i(lim,ltxt)
#Generate the test results. 
print("Image Retrieval Accuracy with best METEOR model is:")
print("r1: {}, r5: {}, r10: {}".format(test_r1, test_r5, test_r10))
...
#Save the translation prediction to the trained_model_path
test_prediction_path = os.path.join(output_path,'test_best_METEOR.'+target_language)
